...
void Antenna::SafeDeploy() const {
    ...
    byte copy_command_mode_sleep = Antenna::kCommandModeSleep;

    bus->PerformWriteTransaction(Antenna::kAddress, &copy_command_mode_sleep,
                                 1);
}
...
AntennaMessage Antenna::GetStatus() const {
    ...
    // TODO(dingbenjamin): Clarify with Stan from Endurosat what a 1/0 means
