...
TEST(TcomCheckout, CanSenseCommsRegulator1OutputVoltage) {
    double voltage =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComOutV1, 0);
    DOUBLES_EQUAL(8.2, voltage, 0.5);
}

TEST(TcomCheckout, CanSenseCommsRegulator1InputVoltage) {
    double voltage =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComInV1, 0);
    DOUBLES_EQUAL(5.2, voltage, 0.5);
}

TEST(TcomCheckout, CanSenseCommsRegulator2OutputVoltage) {
    double voltage =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComOutV2, 0);
    DOUBLES_EQUAL(8.2, voltage, 0.5);
}

TEST(TcomCheckout, CanSenseCommsRegulator2InputVoltage) {
    double voltage =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComInV2, 0);
    DOUBLES_EQUAL(5.2, voltage, 0.5);
}

TEST(TcomCheckout, CanSenseCommsRegulator1InputCurrent) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComInI1, 0);
...
TEST(TcomCheckout, CanSenseCommsRegulator2OutputCurrent) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kComOutI2, 0);
    DOUBLES_EQUAL(1.5, current, 1.5);
}
