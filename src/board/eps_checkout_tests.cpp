...
TEST(EpsCheckout, CanSensePowerTemp1) {
    double temp = i2c_measurable_manager->ReadI2cMeasurable<double>(kPowerTemp1, 0);
    DOUBLES_EQUAL(25, temp, 5);
}

TEST(EpsCheckout, CanSensePowerTemp2) {
    double temp = i2c_measurable_manager->ReadI2cMeasurable<double>(kPowerTemp2, 0);
    DOUBLES_EQUAL(25, temp, 5);
}

TEST(EpsCheckout, CanSensePowerBmsDieTemp1) {
    double temp = i2c_measurable_manager->ReadI2cMeasurable<double>(kPowerBmsDieTemp1, 0);
    DOUBLES_EQUAL(50, temp, 30);
}

TEST(EpsCheckout, CanSensePowerBmsDieTemp2) {
    double temp = i2c_measurable_manager->ReadI2cMeasurable<double>(kPowerBmsDieTemp2, 0);
    DOUBLES_EQUAL(50, temp, 30);
}
