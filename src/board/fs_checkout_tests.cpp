...
TEST(FsCheckout, Imu1) {
    double temperature =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsImuT1, 0);

    DOUBLES_EQUAL(25, temperature, 45);
}

TEST(FsCheckout, Imu2) {
    double temperature =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsImuT2, 0);
...
TEST(FsCheckout, CanSenseFsTorquerCurrentX) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsTorquerXI, 0);
    DOUBLES_EQUAL(0.75, current, 0.76);
}

TEST(FsCheckout, CanSenseFsTorquerCurrentTotal) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsTorquerTotalI, 0);
    DOUBLES_EQUAL(2, current, 2.01);
}

TEST(FsCheckout, CanSenseFsTorquerCurrentY) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsTorquerYI, 0);
    DOUBLES_EQUAL(0.75, current, 0.76);
}

TEST(FsCheckout, CanSenseFsTorquerCurrentZ) {
    double current =
        i2c_measurable_manager->ReadI2cMeasurable<double>(kFsTorquerZI, 0);
    DOUBLES_EQUAL(0.75, current, 0.76);
}
