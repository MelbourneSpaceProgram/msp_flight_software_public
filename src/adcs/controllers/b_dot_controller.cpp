...
void BDotController::Control(const Matrix &b, const Matrix &b_dot,
    ...
    if (!b_dot.SameSize(b) || !b_dot.SameSize(torque_output) ||
        ...
        etl::exception e("Arguments not 3x1", __FILE__,
                         __LINE__);
        throw e;
    }
    ...
    //}
    // Hack attack test time
    double b_magnitude = Matrix::VectorNorm(b);
    for (uint8_t i = 0; i < 3; i++) {
        magnetic_dipole.Set(i, 0, -(b_dot.Get(i, 0) / b_magnitude));
    }
