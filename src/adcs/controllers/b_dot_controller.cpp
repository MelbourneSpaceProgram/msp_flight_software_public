...
const double BDotController::ControllerGains[3] = {30000.0,30000.0,30000.0};
const double BDotController::max_dipole[3] = {0.2,0.2,0.05};
...
void BDotController::ComputeControl(const Matrix &b_dot,
    ...
    for (uint8_t i = 0; i < 3; i++) {
        result[i] = -1*ControllerGains[i]*b_dot.Get(i,0)/max_dipole[i];
    ...
    if (result_abs[0]>1 || result_abs[1]>1 || result_abs[2]>1){
        ...
        double max = *std::max_element(result_abs,result_abs +sizeof(result_abs)/sizeof(double));
