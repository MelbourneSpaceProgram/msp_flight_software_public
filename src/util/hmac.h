...
/// hmac_buffer should be size HashMethod::BlockSize
template <typename HashMethod>
void hmac(byte hmac_buffer[], const byte data[], size_t numDataBytes, const byte key[], size_t numKeyBytes)
{
  ...
  finalHasher.getHash(hmac_buffer);
}

/// compute HMAC hash of data and key using MD5, SHA1 or SHA256
template <typename HashMethod>
std::string hmac(const byte data[], size_t numDataBytes, const byte key[], size_t numKeyBytes)
{
  // initialize key with zeros
  unsigned char usedKey[HashMethod::BlockSize] = {0};
...
{
  return hmac<HashMethod>(reinterpret_cast<const byte*>(data.data()), data.size(), reinterpret_cast<const byte*>(key.data()), key.size());
}
