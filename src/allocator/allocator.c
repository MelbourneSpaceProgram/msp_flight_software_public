...
void print_schedule()
{
    ...
    {
        printf("schedule[%3d] = {%2d, %10d, %10d, %10f}\n", i, schedule.requests[i].priority, schedule.requests[i].start_time, schedule.requests[i].end_time, schedule.requests[i].power_average);
    }
...
allocator_status_type allocator(request_type request)
{
    ...
    //can)=_be_allocated &= power_allocator(rqeuest, list_of_events, n_events_allocated);
