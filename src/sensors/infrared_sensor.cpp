...
InfraredSensor::InfraredSensor(Matrix &side_normal_matrix) :
        side_normal(side_normal_matrix, side_normal_data), infrared_reading(
                infrared_reading), angle_to_nadir(angle_to_nadir)
{
}
...
void InfraredSensor::InfraredToAngle()
{
    ...
    angle_to_nadir = sensor_a_poly_coeffs[0] * pow(x, 9)
            ...
            + sensor_a_poly_coeffs[7] * pow(x, 2) + sensor_a_poly_coeffs[8] * x
            + sensor_a_poly_coeffs[9];
}
...
Matrix InfraredSensor::GetSideNormal() const
{
    return side_normal;
}

double InfraredSensor::GetInfraredReading() const
{
