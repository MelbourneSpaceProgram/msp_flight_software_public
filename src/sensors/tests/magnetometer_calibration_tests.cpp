...
const float
    test_calibration_values_data[MagnetometerCalibration::data_size][3] = {
...
const float B_expected_data[3][1] = {
    {-47.7545590950}, {-30.0125021696}, {-72.0746933014}};

const float D_expected_data[3][3] = {
...
void TestComputeS(void) {
  Matrix<float> test_calibration_values(
                                        const_cast<float(&)[40][3]>(test_calibration_values_data));

  Matrix<float> expected_S(const_cast<float(&)[10][10]>(S_expected_data));
  MagnetometerCalibration magnetometer_calibration;
  magnetometer_calibration.ComputeMatrixS(test_calibration_values);
  for (uint8_t i = 0; i < 10; i++) {
    for (uint8_t j = 0; j < 10; j++) {
      TEST_ASSERT_FLOAT_WITHIN(1e-4, 1,
                               magnetometer_calibration.GetS().Get(i, j) / expected_S.Get(i,j));
    }
  }
}
...
void TestMagnetometerCalibration(void) {
    ...
    Matrix<float> B_expected(const_cast<float(&)[3][1]>(B_expected_data));

    Matrix<float> D_expected(const_cast<float(&)[3][3]>(D_expected_data));
