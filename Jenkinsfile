						...
						node('master') {
							...
							step([
								...
								config: [
									...
									violationConfigs: [ [ pattern: '.*/checker_output/.*\\.xml$', parser: 'CPPCHECK', reporter: 'CPPCHECK' ]]
								]	
							])
						}
					},
					"build": {
						node('AWS_Docker_Agent') {
							sh '''
							tar cf CDH_software.tar.gz --exclude=./CDH_software.tar.gz -C ${WORKSPACE} .
